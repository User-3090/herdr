...
| `herdr --remote` to a pre-provisioned Windows target | beta |
...
Native Windows remote attach uses `ssh.exe`, a local named-pipe bridge, and the existing thin-client protocol. The pre-provisioned target must use PowerShell as its OpenSSH default shell and provide the matching executable at `C:\HerdrSandbox\runtime\herdr\herdr.exe`. Herdr invokes that executable directly, so an installer-junction `.cmd` shim is not required.

The Windows adapter reads target aliases through the normal user and system OpenSSH configuration, then passes a private temporary configuration to `ssh.exe -F` with bounded keepalive defaults. Applications can publish aliases through an `Include` in the user's standard SSH config without a Herdr-specific environment variable.

For other Windows targets, SSH into the server and run `herdr` there:
...
That mode runs Herdr on the remote host. Automatic Windows target discovery and installation are not part of the beta.
