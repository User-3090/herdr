...
Native Windows `herdr --remote` supports pre-provisioned Windows targets in the beta. It uses `ssh.exe`, PowerShell, and the existing thin-client protocol over local named pipes. The target must provide the matching executable at `C:\HerdrSandbox\runtime\herdr\herdr.exe`; automatic Windows target discovery and installation are not supported yet. From Windows to any other target, SSH into the server and run `herdr` there.

For an application-owned SSH alias, publish an `Include` through the user's standard SSH config. The Windows remote adapter includes normal user and system OpenSSH configuration in its private per-attach config before adding fallback keepalive settings.
