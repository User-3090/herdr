...
Native Windows `herdr --remote` supports pre-provisioned Windows targets in the beta. It uses `ssh.exe`, PowerShell, and the existing thin-client protocol over local named pipes. The target must provide the matching executable at `C:\Herdr\herdr.exe`; automatic Windows target discovery and installation are not supported yet. From Windows to any other target, SSH into the server and run `herdr` there.

For an application-owned SSH alias, set `HERDR_SSH_CONFIG` to the configuration file. The Windows remote adapter passes it to `ssh.exe -F`; it does not write to the user's SSH profile.
